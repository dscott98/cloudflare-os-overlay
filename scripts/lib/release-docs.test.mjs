...
const PREV_SHA = 'bdc5c85daf5e9c7fc80799c3fe042cd38e25add0';
const NEW_SHA = 'b09c64cb66c13eb106f5f825e8c5f71636f09eb8';
...
const RELEASES = [
  ...
  '| --- | --- | --- | --- |',
  '| `0.1.0-candidate.8` | `' + PREV_SHA + '` | candidate | Some notes. |',
  '',
...
const README = [
  '[![Upstream Pin](https://img.shields.io/badge/upstream-bdc5c85-orange)](https://github.com/cloudflare/cloudflare-os/tree/' + PREV_SHA + ')',
  '',
...
test('insertReleaseRow places the new row directly under the table header separator', () => {
  ...
  assert.ok(lines[5].includes('0.1.0-candidate.8'), 'existing rows stay below the new row');
});
...
test('nextCandidateVersion increments the newest candidate number', () => {
  assert.equal(nextCandidateVersion(RELEASES), '0.1.0-candidate.9');
...
test('updateReadmeForPin rewrites short and full SHAs and the candidate version', () => {
  const out = updateReadmeForPin(README, {prevSha: PREV_SHA, newSha: NEW_SHA, newCandidateVersion: '0.1.0-candidate.9'});
  assert.ok(out.includes('upstream-b09c64c-orange'), 'badge short SHA updated');
  assert.ok(!out.includes('bdc5c85'), 'no stale short or full SHA remains');
  assert.ok(out.includes(NEW_SHA), 'full SHA updated');
  assert.ok(out.includes('`0.1.0-candidate.9` in'), 'candidate version updated');
});
...
test('applyUpstreamPinToDocs updates UPSTREAM.json, RELEASES.md, and README.md together', () => {
  ...
  try {
    ...
    const result = applyUpstreamPinToDocs({root: dir, prevSha: PREV_SHA, newSha: NEW_SHA, commitCount: 2});

    assert.equal(result.newCandidateVersion, '0.1.0-candidate.9');
    ...
    assert.ok(releases[4].includes('`0.1.0-candidate.9`'), 'new row directly under the separator');
    assert.ok(releases[4].includes(NEW_SHA));

    const readme = fs.readFileSync(path.join(dir, 'README.md'), 'utf8');
    assert.ok(readme.includes(NEW_SHA) && !readme.includes(PREV_SHA));
    assert.ok(readme.includes('`0.1.0-candidate.9` in'));
  } finally {
